...
QSGNode *Line::updatePaintNode(QSGNode *oldNode, UpdatePaintNodeData *)
{
    ...
    float thickness = properties->line()->thickness();
...
QRectF Line::boundingRect() const
{
    ...
    qreal pad = properties->line()->thickness() / 2.0;
...
bool Line::contains(const QPointF &point) const
{
    ...
    if (c1 <= 0)
        return (p - p1).length() <= properties->line()->thickness() / 2.0f;

    float c2 = QVector2D::dotProduct(v, v);
    if (c2 <= c1)
        return (p - p2).length() <= properties->line()->thickness() / 2.0f;
